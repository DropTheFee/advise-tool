...
export default {
  async fetch(request, env) {
    ...
    // PUBLIC — client briefs, no auth ever
...
if (path === '/do-login' && method === 'POST') return handleLogin(request, env);
    ...
    // API — sessions
    if (path === '/api/save' && method === 'POST') return handleSave(request, env, cors);
    if (path.startsWith('/api/session/') && method === 'GET') return handleGetSession(path.replace('/api/session/',''), env, cors);
    if (path.startsWith('/api/brief/') && method === 'POST') return handleGenerateBrief(path.replace('/api/brief/',''), 'client', request, env, cors);
    if (path.startsWith('/api/admin-brief/') && method === 'POST') return handleGenerateBrief(path.replace('/api/admin-brief/',''), 'admin', request, env, cors);
    if (path === '/api/sessions' && method === 'GET') return handleListSessions(request, env, cors);
    ...
    if (path.startsWith('/api/session/') && method === 'DELETE') return handleDeleteSession(path.replace('/api/session/',''), env, cors);
    if (path.startsWith('/api/lock/') && method === 'POST') return handleLockSession(path.replace('/api/lock/',''), request, env, cors);

    // API — user management (admin only)
    if (path === '/api/users' && method === 'GET') return handleListUsers(request, env, cors);
    if (path === '/api/users' && method === 'POST') return handleCreateUser(request, env, cors);
    if (path.startsWith('/api/users/') && path.endsWith('/reset') && method === 'POST') return handleResetPassword(path.replace('/api/users/','').replace('/reset',''), request, env, cors);
    if (path.startsWith('/api/users/') && method === 'PUT') return handleUpdateUser(path.replace('/api/users/',''), request, env, cors);
    if (path.startsWith('/api/users/') && method === 'DELETE') return handleDeleteUser(path.replace('/api/users/',''), request, env, cors);

    // PROTECTED — admin brief + user management page
    if (path.startsWith('/admin/')) {
      var sess = await getAuthSession(request);
      if (!sess) return redir('/login');
      if (path === '/admin/users') return serveAsset('users.html', env);
      return serveBriefPage(path.replace('/admin/',''), env, 'admin');
    }

    // PROTECTED — tool + dashboard
    var sess = await getAuthSession(request);
    if (!sess) {
      return (await usersExist(env)) ? redir('/login') : redir('/setup');
    }
    if (sess.mustChangePassword && path !== '/change-password') return redir('/change-password');
...
async function handleSetup(request, env, cors) {
  try {
    ...
    if (!username||!password||!displayName||!email) return jsonResp({ error:'All fields required' }, 400, cors);
    if (password.length < 8) return jsonResp({ error:'Password must be at least 8 characters' }, 400, cors);
    var hash = await hashPassword(password);
    var now = new Date().toISOString();
    await saveUser(username, { username, displayName, email, phone, role:'admin', disabled:false, createdAt:now, lastLogin:null }, env);
...
async function handleLogin(request, env) {
  ...
  if (!profile || !auth || profile.disabled || !(await verifyPassword(password, auth.passwordHash))) {
    return redir('/login?error=' + encodeURIComponent('Invalid username or password.'));
  }
  profile.lastLogin = new Date().toISOString();
  await saveUser(username, profile, env);
  var token = await signToken({ username:profile.username, role:profile.role, displayName:profile.displayName, phone:profile.phone, email:profile.email, mustChangePassword:auth.mustChangePassword||false, exp:Date.now()+(COOKIE_TTL*1000) });
  return new Response(null, { status:302, headers:{ 'Location':auth.mustChangePassword?'/change-password':'/', 'Set-Cookie':setCookie(token) } });
}
...
async function handleChangePassword(request, env, cors) {
  try {
    ...
    var token = await signToken({ username:sess.username, role:sess.role, displayName:sess.displayName, phone:profile?profile.phone:'', email:profile?profile.email:'', mustChangePassword:false, exp:Date.now()+(COOKIE_TTL*1000) });
    return new Response(JSON.stringify({ ok:true }), { status:200, headers:Object.assign({},cors,{'Content-Type':'application/json','Set-Cookie':setCookie(token)}) });
  } catch(e) { return jsonResp({ error:e.message }, 500, cors); }
}
...
async function handleCreateUser(request, env, cors) {
  ...
  try {
    ...
    var now = new Date().toISOString();
    await saveUser(username, { username, displayName, email, phone, role, disabled:false, createdAt:now, lastLogin:null }, env);
...
async function handleSave(request, env, cors) {
  try {
    var body = await request.json();
    var { sessionId, data } = body;
    if (!sessionId||!data) return jsonResp({ error:'Missing fields' }, 400, cors);
    var sess = await getAuthSession(request);
    if (sess && !data.createdBy) { data.createdBy=sess.username; data.createdByName=sess.displayName; data.repPhone=sess.phone; data.repEmail=sess.email; }
    await env.ADVISE_SESSIONS.put('session:'+sessionId+':data', JSON.stringify(data), { expirationTtl:60*60*24*90 });
    var meta = { id:sessionId, prospect:data.prospect||{}, tier:data.selectedTier||'', outcome:data.outcome||'', totalLeak:data.calc?data.calc.totalAnn:0, createdBy:data.createdBy||'unknown', createdByName:data.createdByName||'Unknown', repPhone:data.repPhone||'', repEmail:data.repEmail||'', createdAt:data.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString(), briefGenerated:false, locked:false };
    var ex = await env.ADVISE_SESSIONS.get('session:'+sessionId+':meta');
    if (ex) { var e=JSON.parse(ex); meta.locked=e.locked||false; meta.briefGenerated=e.briefGenerated||false; meta.briefUrl=e.briefUrl||''; meta.adminUrl=e.adminUrl||''; meta.createdAt=e.createdAt||meta.createdAt; meta.createdBy=e.createdBy||meta.createdBy; meta.createdByName=e.createdByName||meta.createdByName; meta.repPhone=e.repPhone||meta.repPhone; meta.repEmail=e.repEmail||meta.repEmail; }
    await env.ADVISE_SESSIONS.put('session:'+sessionId+':meta', JSON.stringify(meta), { expirationTtl:60*60*24*90 });
    return jsonResp({ ok:true, sessionId }, 200, cors);
  } catch(e) { return jsonResp({ error:e.message }, 500, cors); }
}
  ...
  try {
    ...
    for (var k of list.keys) {
      if (k.name.endsWith(':meta')) {
        var r = await env.ADVISE_SESSIONS.get(k.name);
        if (r) { var m=JSON.parse(r); if (!sess||sess.role==='admin'||m.createdBy===sess.username) metas.push(m); }
      }
    }
...
async function handleGenerateBrief(id, briefType, request, env, cors) {
  try {
    ...
    var briefUrl = briefType==='admin'?'https://advise.surj.app/admin/'+id:'https://advise.surj.app/brief/'+id;
    var mr = await env.ADVISE_SESSIONS.get('session:'+id+':meta');
    if (mr) { var m=JSON.parse(mr); m.briefGenerated=true; if(briefType==='client')m.briefUrl=briefUrl; if(briefType==='admin')m.adminUrl=briefUrl; await env.ADVISE_SESSIONS.put('session:'+id+':meta',JSON.stringify(m)); }
    fireGHLIntegration(body.sessionData, briefUrl).catch(function(){});
    return jsonResp({ ok:true, briefUrl }, 200, cors);
  } catch(e) { return jsonResp({ error:e.message }, 500, cors); }
}
...
async function handleLockSession(id, request, env, cors) {
  try {
    ...
    var m = JSON.parse(mr); m.locked = body.locked;
...
async function serveBriefPage(id, env, briefType) {
  ...
  try {
    ...
    if (brief) return new Response(brief, { headers:{'Content-Type':'text/html;charset=UTF-8'} });
    var rep = { displayName:'Steve Wilson', phone:'(405) 913-1956', email:'admin@surj.app' };
    var mr = await env.ADVISE_SESSIONS.get('session:'+id+':meta');
    if (mr) { var m=JSON.parse(mr); if(m.createdByName)rep.displayName=m.createdByName; if(m.repPhone)rep.phone=m.repPhone; if(m.repEmail)rep.email=m.repEmail; }
...
async function serveAsset(filename, env) {
  try { return await env.ASSETS.fetch('https://assets/' + filename); }
  catch(e) { return new Response('Not found: ' + filename, { status:404 }); }
}

// ─── GHL ──────────────────────────────────────────────────────────────────────
async function fireGHLIntegration(sessionData, briefUrl) {
  if (!sessionData) return;
  var p=sessionData.prospect||{}, calc=sessionData.calc||{}, tier=sessionData.selectedTier||'';
  ...
  var contactId=null;
  try { var s=await (await fetch(GHL_API_BASE+'/contacts/?locationId='+GHL_LOCATION_ID+'&query='+encodeURIComponent((p.first||'')+' '+(p.last||'')),{headers:h})).json(); if(s.contacts&&s.contacts.length>0)contactId=s.contacts[0].id; } catch(e){}
  ...
  try { if(contactId){await fetch(GHL_API_BASE+'/contacts/'+contactId,{method:'PUT',headers:h,body:JSON.stringify(cp)});}else{var cr=await(await fetch(GHL_API_BASE+'/contacts/',{method:'POST',headers:h,body:JSON.stringify(cp)})).json();contactId=cr.contact?cr.contact.id:null;} } catch(e){}
  if(!contactId)return;
  var note=['=== ADVISE Call — '+new Date().toLocaleDateString()+' ===','Rep: '+(sessionData.createdByName||'Unknown'),'Business: '+(p.business||'—'),'','Monthly Leak: '+fmtNum(calc.totalMo||0),'Annual Leak: '+fmtNum(calc.totalAnn||0),'Plan: SüRJ '+tierName+' ('+(tierPrices[tier]||'TBD')+'/mo)','','Brief: '+briefUrl].join('\n');
  try { await fetch(GHL_API_BASE+'/contacts/'+contactId+'/notes',{method:'POST',headers:h,body:JSON.stringify({body:note,userId:''})}); } catch(e){}
}
